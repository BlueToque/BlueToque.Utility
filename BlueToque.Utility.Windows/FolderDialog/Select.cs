...
namespace BlueToque.Utility.Windows.FolderDialog
{
    public class Select : ISelect
    {
        ...
        public DialogResult ShowDialog() => ShowDialog(owner: new WindowWrapper(IntPtr.Zero));

        public DialogResult ShowDialog(IWin32Window owner) => Environment.OSVersion.Version.Major >= 6 ? ShowVistaDialog(owner) : ShowLegacyDialog(owner);
        ...
        public DialogResult ShowVistaDialog(IWin32Window owner)
        {
            ...
            if (frm.Show(owner.Handle) == NativeMethods.S_OK)
            {
                ...
                {
                    if (shellItem.GetDisplayName(NativeMethods.SIGDN_FILESYSPATH, out nint pszString) == NativeMethods.S_OK)
                    {
    ...
    public class WindowWrapper(IntPtr handle) : System.Windows.Forms.IWin32Window
    {
        ...
        private readonly IntPtr _hwnd = handle;
    }
    ...
    internal static class NativeMethods
    {
        ...
        TypeLibType(TypeLibTypeFlags.FCanCreate), Guid("DC1C5A9C-E88A-4DDE-A5A1-60F82A20AEF7")]
